...
const statusConfig = {
  ...
  overdue: { label: "Overdue", color: "bg-red-500", textColor: "text-red-400" },
  cancelled: { label: "Cancelled", color: "bg-gray-500", textColor: "text-gray-400" },
}
      ...
      <motion.div {...fadeUp} className="mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button variant="outline" className="gap-2 border-border/50">
            Contact Counterparty <ExternalLink className="h-3.5 w-3.5" />
          </Button>
        </div>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
            ...
            <Card className="border-border/50">
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                  {[
                    { label: "Amount", value: `${txn.amount.toLocaleString()} ${txn.currency}` },
                    { label: "Annual Rate", value: `${txn.interestRate}%`, cls: "text-emerald-400" },
                    { label: "Term", value: `${txn.term} days` },
                    { label: "Repayment", value: txn.repaymentMethod },
                    { label: "Created", value: txn.createdAt },
                    { label: "Due Date", value: txn.expiresAt },
                  ].map((item) => (
          ...
          <motion.div {...fadeUp} transition={{ delay: 0.2 }}>
            <Card className="border-border/50">
              <CardHeader className="pb-3">
                <CardTitle className="text-base">Repayment Timeline</CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex-1 rounded-lg border border-border/30 bg-secondary/20 p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="font-medium">Installment #{r.number}</span>
                            <Badge
                              variant={r.status === "paid" ? "secondary" : (r.status as string) === "overdue" ? "destructive" : "outline"}
                              className="text-xs"
                            >
                              {r.status === "paid" ? "✅ Paid" : (r.status as string) === "overdue" ? "🔴 Overdue" : "⏳ Pending"}
                            </Badge>
                          </div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-4 text-xs text-muted-foreground">
                          ...
                          <span>Principal: ${r.principal.toLocaleString()}</span>
                          <span>Interest: ${r.interest.toLocaleString()}</span>
                          {r.paidAt && <span className="text-emerald-400">Paid on: {r.paidAt}</span>}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <Card className="border-border/50">
              ...
              <CardContent>
                <div className="flex items-center justify-between overflow-x-auto py-4">
                  {[
                    { icon: Wallet, label: "Lender Wallet", sub: "L-****8832", color: "text-emerald-400", bg: "bg-emerald-500/10" },
                    { icon: Shield, label: "Smart Contract", sub: "Escrow", color: "text-violet-400", bg: "bg-violet-500/10" },
                    { icon: CreditCard, label: "Borrower Wallet", sub: txn.counterparty.name, color: "text-cyan-400", bg: "bg-cyan-500/10" },
                    { icon: ArrowRight, label: "Repayment Route", sub: "Auto-debit", color: "text-amber-400", bg: "bg-amber-500/10" },
                  ].map((node, i) => (
          ...
          <motion.div {...fadeUp} transition={{ delay: 0.25 }}>
            <Card className="border-border/50 bg-gradient-to-br from-emerald-500/5 to-transparent">
              <CardContent className="p-5 text-center">
                <div className="mb-2 text-sm text-muted-foreground">Next Payment</div>
